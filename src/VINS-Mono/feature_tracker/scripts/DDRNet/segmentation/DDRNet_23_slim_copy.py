...
def DualResNet_imagenet(pretrained=False, root='./weights/best_val_smaller.pth', map_location='cpu'):
    model = DualResNet(BasicBlock, [2, 2, 2, 2], num_classes=19, planes=32, spp_planes=128, head_planes=64, augment=True)
    if pretrained:
        pretrained_state = torch.load(root, map_location=map_location) 
        model_dict = model.state_dict()
        new_state_dict = OrderedDict()
        ...
        model_dict.update(new_state_dict)     
        model.load_state_dict(model_dict,)
        print("load pretrained model, path is: %s"%root)
    return model

def get_seg_model(pretrained=False, root='./weights/best_val_smaller.pth', map_location='cpu'):

    model = DualResNet_imagenet(pretrained, root, map_location)
    return model
